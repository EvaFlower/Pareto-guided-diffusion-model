...
from models import (anneal_Langevin_dynamics,
                    ...
                    anneal_Langevin_dynamics_single_constraint,
                    anneal_Langevin_dynamics_proud_diverisity,
                    anneal_Langevin_dynamics_mgd_diverisity,
                    anneal_Langevin_dynamics_m1_mgd,
                    anneal_Langevin_dynamics_linear_sum_diversity,
                    anneal_Langevin_dynamics_b_kl_mgd_diverisity,
                    anneal_Langevin_dynamics_b_mgd_kl_diverisity,
                    mgd_only_diversity)
    ...
    def sample(self):
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                print(init_samples.max(), init_samples.min())
                if self.args.guidance == 'none':
                    all_samples = anneal_Langevin_dynamics(init_samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise,
                                                        test_pf=self.args.test_pf)
                elif self.args.guidance == 'control_1':
                    all_samples = anneal_Langevin_dynamics_single_constraint(init_samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise)
                elif self.args.guidance == 'proud_diversity':
                    all_samples = anneal_Langevin_dynamics_proud_diverisity(init_samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                                                        test_pf=self.args.test_pf,
                                                        lambda_diver=self.args.lambda_diversity)
                elif self.args.guidance == 'mgd_diversity':
                    all_samples = anneal_Langevin_dynamics_mgd_diverisity(init_samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                                                        test_pf=self.args.test_pf,
                                                        lambda_diver=self.args.lambda_diversity,
                                                        lambda_mgd=self.args.lambda_mgd)
                elif self.args.guidance == 'm1_mgd':
                    all_samples = anneal_Langevin_dynamics_m1_mgd(init_samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                                                        test_pf=self.args.test_pf)
                elif self.args.guidance == 'linear_sum_diversity':
                    all_samples = anneal_Langevin_dynamics_linear_sum_diversity(init_samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                                                        test_pf=self.args.test_pf,
                                                        lambda_diver=self.args.lambda_diversity,
                                                        lambda_f=self.args.lambda_f,
                                                        w1=self.args.linear_sum_w1)
                elif self.args.guidance == 'b_kl_mgd_diversity':
                    all_samples = anneal_Langevin_dynamics_b_kl_mgd_diverisity(init_samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                                                        test_pf=self.args.test_pf)
                elif self.args.guidance == 'b_mgd_kl_diversity':
                    all_samples = anneal_Langevin_dynamics_b_mgd_kl_diverisity(init_samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                                                        test_pf=self.args.test_pf)
                elif self.args.guidance == 'mgd_only_diversity':
                    all_samples = mgd_only_diversity(init_samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                    ...
                    print('inappropriate guidance method...')
                    exit()
            ...
            for _ in tqdm.tqdm(range(n_rounds), desc='Generating image samples for FID/inception score evaluation'):
                ...
                if self.args.guidance is None:
                    ...
                    all_samples = anneal_Langevin_dynamics_single_constraint(samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise)
                elif self.args.guidance == 'proud_diversity':
                    all_samples = anneal_Langevin_dynamics_proud_diverisity(samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                    ...
                    all_samples = anneal_Langevin_dynamics_mgd_diverisity(samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                    ...
                    all_samples = anneal_Langevin_dynamics_m1_mgd(samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                    ...
                    all_samples = anneal_Langevin_dynamics_linear_sum_diversity(samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                    ...
                    all_samples = anneal_Langevin_dynamics_b_kl_mgd_diverisity(samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                    ...
                    all_samples = anneal_Langevin_dynamics_b_mgd_kl_diverisity(samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
                                                        test_pf=self.args.test_pf)
                elif self.args.guidance == 'mgd_only':
                    all_samples = mgd_only_diversity(samples, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr, verbose=True,
                                                        final_only=self.config.sampling.final_only,
                                                        denoise=self.config.sampling.denoise, 
