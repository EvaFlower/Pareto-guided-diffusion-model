...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    #covmean = tf.linalg.sqrtm(sigma1.dot(sigma2), name=None)
...
def calculate_fid_given_files(imgs, stats_path, inception_path=None, low_profile=False):
    ''' Calculates the FID of two paths. '''
    inception_path = check_or_download_inception(inception_path)
    ...
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        m1, s1 = _handle_path(stats_path, sess, low_profile=low_profile)
        m2, s2 = calculate_activation_statistics(imgs, sess) 
        fid_value = calculate_frechet_distance(m1, s1, m2, s2)
        return fid_value
